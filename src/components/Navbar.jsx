...
export default function Navbar() {
  ...
  return (
    <>
        ...
        <nav 
        ref={navRef}
        className='flex flex-col fixed z-50 w-full h-full px-10 uppercase gap-y-10 md:w-1/2 bg-black text-white/80 justify-between py-28'>
            {/* nav links */}
        ...
        className="fixed z-50 flex flex-col items-center justify-center gap-1 transition-all duration-300 bg-black rounded-full cursor-pointer w-14 h-14 md:w-20 md:h-20 top-4 right-10">
            <span ref={topLineRef} className='block rounded-full w-8 h-0.5 bg-white'></span>
            <span ref={bottomLineRef} className='block rounded-full w-8 h-0.5 bg-white'></span>
        </div>
    </>
  )
}
